		...
		delete.addActionListener( new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) //点击触发删除事件
			{
				...
				try
				{
					...
					st = new Connect().st;
					query = "delete from user_flight where flight_id = "+String.valueOf(flight_id);
					st.executeUpdate(query);	
					
					//更新flight表中status ，即飞机座位情况
					//{{
	...
	public void gettable()
	{
		try 
		{
			...
			while( rs.next() )
			{
				...
				//starttime.substring(beginIndex, endIndex)
