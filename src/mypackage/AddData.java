...
public class AddData extends JFrame implements ActionListener
{
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		else if( e.getSource().equals(bt[4]) )	//添加航班
		{
			...
			lb[3].setText("出发时间：例如（2018-01-01 09:30:00）");
			lb[4].setText("到达时间:(格式同上)");
			lb[5].setText("价格");
			lb[6].setText("出发地");
			lb[7].setText("目的地");
			lb[8].setText("出发地使用的机场编号（编号在数据库中一定要有）");
			lb[9].setText("目的地机场编号（编号在数据库中一定要有）");
			for( int i = 1 ; i <= 9 ; i++ )
			{
				lb[i].setVisible(true);
				txt[i].setVisible(true);
			...
			else if( lastbutton == 4 )	//添加航班
			{
				...
				else
				{
					...
					query = "insert into flight values"
							...
							+"'"+startplace+"',"+"'"+endplace+"','"+status+"')";
					//System.out.println("flight :query "+query);
					st.executeUpdate(query);
					step3 = true;
					new Tip("添加成功");
					cleartxt();
